<template>
  <div class="cl-table-wrapper">
    <div class="cl-table">
      <!-- Header -->
      <div class="cl-th">
        <div class="cl-th-cell svc">服务 / 环境</div>
        <div class="cl-th-cell ver">版本</div>
        <div class="cl-th-cell preview">配置预览</div>
        <div class="cl-th-cell changes">变更</div>
        <div class="cl-th-cell time">最近发布</div>
        <div class="cl-th-cell status">状态</div>
      </div>
      ...
      <div v-else class="cl-body">
        ...
        >
          ...
          <div class="cl-cell ver">
            <span class="ver-num">{{ versionLabel(row) }}</span>
          </div>
...
function importantChanges(pub, draft) {
  return KEY_LABELS.flatMap(({ keys, label }) => {
    ...
    const oldVal = hasOwn(pub, key) ? (pub[key] || '空值') : '新增'
    const newVal = hasOwn(draft, key) ? (draft[key] || '空值') : '删除'
...
function isGoZeroTime(t) {
  if (!t) return true
  const d = new Date(t)
  if (isNaN(d.getTime())) return true
  // Go zero time is year 0001; anything before year 2000 is clearly invalid
  if (d.getFullYear() < 2000) return true
  return false
}

function formatRelative(t) {
  if (!t || isGoZeroTime(t)) return '—'
  ...
  if (days > 36500) return '—'
  return `${days} 天前`
}
</script>

<style scoped>
.cl-table-wrapper { flex: 1; overflow-y: auto; background: var(--bg-surface); }
.cl-table { min-width: 100%; }
...
.ver { width: 8%; min-width: 50px; text-align: center; }
...
/* Cells */
.cl-cell { font-size: var(--font-size-sm); color: var(--text-primary); overflow: hidden; white-space: nowrap; }
.svc-name { font-weight: 500; color: var(--text-primary); }
.svc-env {
...
.ver-num { font-variant-numeric: tabular-nums; color: var(--text-secondary); }
...
.no-changes { color: var(--text-placeholder); }
.time-text { font-size: var(--font-size-xs); color: var(--text-placeholder); }
...
.empty-title { font-size: var(--font-size-base); font-weight: 500; color: var(--text-primary); margin-bottom: 4px; }
.empty-desc { font-size: var(--font-size-xs); color: var(--text-secondary); }
</style>
