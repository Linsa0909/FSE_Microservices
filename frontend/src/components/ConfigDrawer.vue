        ...
        <div class="dp-table">
          ...
          <div v-for="diff in diffRows" :key="diff.key" class="dp-row" :class="diff.rowClass">
            ...
            <div class="dp-cell draft">
              <input v-if="diff.editing" v-model="diff.editValue" class="dp-inline-input"
                @keydown.enter="saveInline(diff)" @keydown.escape="cancelInline(diff)" @blur="saveInline(diff)" />
              <span v-else class="dp-editable" :class="{ added: diff.added, deleted: diff.deleted }" @click="startEdit(diff)">
                {{ diff.draftVal || '—' }}
              </span>
            </div>
          ...
          <div class="dp-row add-row">
            <div class="dp-cell key">
              <input v-model="newKey" class="dp-inline-input" placeholder="new.key" @keydown.enter="addKey" />
            </div>
...
const diffRows = computed(() => {
  ...
  return allKeys.map(key => {
    ...
    return { key, publishedVal: pubVal, draftVal, originalDraftVal: draftVal, added, deleted, modified, rowClass, editing: false, editValue: draftVal || '' }
  })
})
...
async function addKey() {
  ...
  if (!k) return
  try {
    await setKey(props.service, props.env, k, v)
    newKey.value = ''
    newValue.value = ''
    await loadDetail()
...
<style scoped>
.detail-panel { width: 680px; min-width: 680px; height: 100%; background: var(--bg-surface); border-left: 1px solid var(--border-subtle); display: flex; flex-direction: column; overflow: hidden; }
...
.added { color: var(--diff-added-text); } .deleted { color: var(--diff-deleted-text); text-decoration: line-through; } .muted { color: var(--text-placeholder); }
