...
# 1. 启动后端（配置中心）
echo "[1/6] 启动配置中心后端 (:8080)..."
(cd "$(dirname "$0")/backend" && go run .) &
BACKEND_PID=$!
sleep 2

# 2. 启动默认示例服务 (order-service, 配置回显)
echo "[2/6] 启动默认示例服务 (:3000)..."
...
# 6. 启动前端
echo "[6/6] 启动配置中心前端 (:5173)..."
(cd "$(dirname "$0")/frontend" && npm run dev) &
FRONTEND_PID=$!

trap 'kill $BACKEND_PID $DEFAULT_PID $RADAR_PID $SENSOR_PID $NAV_PID $FRONTEND_PID 2>/dev/null' EXIT
